...
def get_token_id(file_name):
    "Чтение токена и id пользователя из файла vk_token"
    with open(os.path.join(os.getcwd(), 'vk_token.txt'), 'r') as token_file:
...
def find_max_dpi(dict_in_search):
    "Возвращение ссылку на фото максимального размера и размер фото"
    max_dpi = 0
    need_elem = 0
...
class VkRequest:
    ...
    def _get_photo_info(self):
        "Получение количества фотографий и массива фотографий"
    ...
    def _sort_info(self):
        "Получение словаря с параметрами фотографий и списка JSON"
        json_list = []
        sorted_dict = {}
...
class Yandex:
    def __init__(self, folder_name, token_list, num=5):
        """Получение основных параметров для загрузки фотографий на Я-диск"""
        self.token = token
    ...
    def _create_folder(self, folder_name):
        "Создание папки на Я-диске для загрузки фотографий"
    ...
    def _in_folder(self, folder_name):
        "Получение ссылки для загрузки на Я-диск"
        url = "https://cloud-api.yandex.net/v1/disk/resources"
        params = {'path': folder_name}
